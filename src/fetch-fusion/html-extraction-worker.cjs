...
function extract(html, url) {
  ...
  try {
    ...
    const officialUrls = Array.from(document.querySelectorAll('a[href]')).map((anchor) => anchor.getAttribute('href') || '')
    ...
    return {
      ...
      content: clean(articleText || rawText),
...
parentPort.on('message', (message) => {
  try {
    parentPort.postMessage({ id: message.id, result: extract(message.html, message.url) });
  } catch (error) {
